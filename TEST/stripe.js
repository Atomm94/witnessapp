...
const customerId = "cus_Ixew17xHNcuOYk";
...
router.post("/createCharge", async (req, res) => {
    ...
    //         });
    //     }
    const { cardNumber, cardExpMonth, cardExpYear, cardCVC, country, postalCode, amount, email } = req.body;
        try {
            const cardToken = await stripe.tokens.create({
                card: {
                    number: cardNumber,
